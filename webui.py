...
inference_mode_list = ['Pretrained Voice', '3s Voice Clone', 'Cross-lingual Clone', 'Natural Language Control']
instruct_dict = {'Pretrained Voice': '1. Select a pretrained voice\n2. Click the generate audio button',
                 '3s Voice Clone': '1. Select prompt audio file or record prompt audio (max 30s, file takes priority if both provided)\n2. Enter prompt text\n3. Click the generate audio button',
                 'Cross-lingual Clone': '1. Select prompt audio file or record prompt audio (max 30s, file takes priority if both provided)\n2. Click the generate audio button',
...
def generate_audio(tts_text, mode_checkbox_group, sft_dropdown, prompt_text, prompt_wav_upload, prompt_wav_record, instruct_text,
    ...
    if mode_checkbox_group in ['Cross-lingual Clone']:
        if instruct_text != '':
            gr.Info('You are using Cross-lingual Clone mode, instruction text will be ignored')
        if prompt_wav is None:
            gr.Warning('You are using Cross-lingual Clone mode, please provide prompt audio')
            yield (cosyvoice.sample_rate, default_data)
        gr.Info('You are using Cross-lingual Clone mode, please ensure synthesis text and prompt text are in different languages')
    ...
    if mode_checkbox_group in ['Pretrained Voice']:
        if instruct_text != '' or prompt_wav is not None or prompt_text != '':
            gr.Info('You are using Pretrained Voice mode, prompt text/prompt audio/instruction text will be ignored!')
        if sft_dropdown == '':
    ...
    if mode_checkbox_group in ['3s Voice Clone']:
        if prompt_text == '':
            ...
            yield (cosyvoice.sample_rate, default_data)
        if instruct_text != '':
            gr.Info('You are using 3s Voice Clone mode, pretrained voice/instruction text will be ignored!')
...
def main():
    with gr.Blocks() as demo:
        ...
        gr.Markdown("#### Enter the text to synthesize, select the inference mode, and follow the instructions")

        tts_text = gr.Textbox(label="Input Text to Synthesize", lines=1, value="I am a generative speech model developed by the FunAudioLLM team, providing comfortable and natural speech synthesis capabilities.")
        with gr.Row():
            mode_checkbox_group = gr.Radio(choices=inference_mode_list, label='Select Inference Mode', value=inference_mode_list[0])
            instruction_text = gr.Text(label="Instructions", value=instruct_dict[inference_mode_list[0]], scale=2)
            sft_dropdown = gr.Dropdown(choices=sft_spk, label='Select Pretrained Voice', value=sft_spk[0], scale=1)
            stream = gr.Radio(choices=stream_mode_list, label='Enable Streaming', value=stream_mode_list[0][1])
        ...
        prompt_text = gr.Textbox(label="Input Prompt Text", lines=1, placeholder="Enter prompt text that matches the prompt audio content (automatic recognition not supported yet)...", value='')
        instruct_text = gr.Textbox(label="Input Instruction Text", lines=1, placeholder="Enter instruction text.", value='')
